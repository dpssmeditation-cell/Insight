...
export const Footer: React.FC<FooterProps> = ({ language, onNavigate, viewerCount }) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            {viewerCount !== undefined && (
              <div className="mb-4 inline-flex items-center gap-2 bg-slate-800/50 px-3 py-1.5 rounded-full border border-slate-700/50">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
                </span>
                <span className="text-xs font-medium text-slate-400">
                  {viewerCount} {language === 'zh' ? '在线' : (language === 'kh' ? 'កំពុងមើល' : 'Online')}
                </span>
              </div>
            )}
